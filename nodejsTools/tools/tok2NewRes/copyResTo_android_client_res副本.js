...
var scanPath= '../client/res_xiaotu';
var output_directory = "../client/res_pkmlp";
...
function formatComm(cb) {
    ...
    var dirList = ToolsHelper.getFileList(scanPath, ['.png', '.jpg', '.JPG']);
    console.log("一个有" + dirList.length +"个图片需要转换");
    var commTemplate = "etcpack '%s' '%s' -c etc1 -aa";
    var array = [];
    ...
    // return;
    async.eachSeries(array, function (commond, cb123) {
    ...
    //
    //
    // exec(_.first(array, 10).join("&&"), function(error, stdout, stderr) {
    //     // sys.print('stdout： ' + stdout);
    //     // sys.print('stderr： ' + stderr);
    //     if (!error) {
    //         console.log('图片转换完成,总共耗时:' + (new Date() - date) + "毫秒");
